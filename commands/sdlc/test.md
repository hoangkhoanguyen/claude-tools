...
`.sdlc/<sprint>/visual-baseline/`. Viết test và CHẠY thật đến khi xanh. Mọi AC/EC/NFR/DAC phải có test hoặc
được liệt kê verify-tay. Ghi `.sdlc/<sprint>/test-report.md`.
...
quan + NFR check + design fidelity check (nếu có UI) + quét hardcode/TODO/unhandled error. Chỉ khi sạch mới
trình bày Pre-manual Report:
