...
0. **Nạp context (nguyên tắc 0)**: đọc `.sdlc/<version>/context.md` — khớp fingerprint thì dùng luôn,
   lệch/thiếu thì chưng cất lại theo `templates/context.template.md`. Đọc `.sdlc/architecture.md`.
   Truyền **đường dẫn** `context.md` cho mọi subagent; agent KHÔNG tự Glob repo tìm `CLAUDE.md`.
1. **Skill dùng được + service ngoài**: đọc từ `context.md` (bảng "Skill / command / agent CỦA DỰ ÁN" và
   "Service ngoài"). KHÔNG quét lại `.claude/skills` hay config dự án.
2. Chỉ khi `context.md` thiếu/`n/a` mà sprint rõ ràng cần → suy ra từ `docker-compose.yml`, `.env.example`,
...
5. **Migration**: nếu sprint đổi schema → dùng lệnh migrate ghi trong `context.md` và chạy trước khi test.
   Ghi vào state.
